...
async def analyze_sentiment(comment: CleanedComment, semaphore: asyncio.Semaphore):
    async with semaphore:
        ...
        response = await llm_with_structure.ainvoke(
            [
                SystemMessage(content="Analyze the sentiment of the following comment. Return the sentiment as a string. Only return the sentiment, no other text. The sentiment should be one of the following: positive, negative, neutral."),
                HumanMessage(
                    content=f"Here is the comment: {comment.cleanedComment}"
                ),
            ]
        )
